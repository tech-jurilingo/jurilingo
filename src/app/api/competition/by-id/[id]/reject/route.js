...
export async function POST(request, { params }) {
  try {
    ...
    // Update status to rejected (or remove from waitlist)
    waitlistItem.status = 'rejected';
    
    // Or to completely remove: competition.waitlist.pull(waitlistItemId);
    ...
    return NextResponse.json(
      { success: false, message: 'Server error while rejecting participant' },
      { status: 500 }
    );
  }
}